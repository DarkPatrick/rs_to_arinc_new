...
uses
  set_mutex, Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants,
...
type
  TForm1 = class(TForm)
    ...
    procedure com_portException(sender: TObject; TComException: TComExceptions;
...
begin
  ...
  begin
    form1.received_data.cells[2, i] :=
      'ERROR: wrong package length - ' + intToStr(length(data_str))
        + ' (must be ' + intToStr(PACKAGE_LEN) + ')';
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if ((port_num2.items[i] = extra_com) and (cancel_extra = FALSE)) then
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      while (len > 0) do
      begin
        cells[prev_cell.X, prev_cell.Y] := '0'
...
begin
  ...
  begin
    ...
    begin
      ...
      com_port.baudRate := strToBaudRate(
        speed_select.items.strings[speed_select.itemIndex]
      );
      if (port_num1.itemIndex = -1) then
      begin
        showMessage('ошибка: порт не настроен. измени конфигурацию');
...
begin
  if ((received_data.row > 0)
    and (rcv_dat_str.cells[0, received_data.row - 1] <> '')) then
  begin
...
begin
  if ((sended_data.row > 0)
    and (trm_dat_str.cells[0, sended_data.row - 1] <> '')) then
  begin
    ...
    begin
      ...
      begin
        ...
        cells[1, i] := int_to_bin(arinc_ch_num.itemIndex + 1, 8)
          + '000000000000000000'
            + int_to_bin(arinc_parity_type.itemIndex, 1)
              + int_to_bin(1 - arinc_parity_en.itemIndex, 1)
...
begin
  ...
  begin
    ...
    while (i > 0) do
    begin
      cells[prev_cell.x, prev_cell.y] := '0' + cells[prev_cell.x, prev_cell.y];
      i := i - 1;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (speed_select.items.strings[i]
          = baudRateToStr(com_port.baudRate)) then
        begin
...
begin
  ...
  begin
    ...
    begin
      cnctd := -1;
      showMessage('Соединение с портом ' + com_port.port + ' было разорвано.');
      com_open_btn.enabled  := FALSE;
      chose_ports.enabled   := FALSE;
      com_close_btn.enabled := FALSE;
      send_pack_btn.enabled := FALSE;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          data_s_grid.cells[1, i] := ansiUpperCase(data_s_grid.cells[1, i]);
          data_s_grid.cells[2, i] := ansiUpperCase(data_s_grid.cells[2, i]);
        end;
        str1 := prepareFirstPackPart();
        ...
        begin
          ...
          str1 := str1
            + ansiUpperCase(str3[2] + str3[3] + str3[4] + str3[5] + str3[6]);
